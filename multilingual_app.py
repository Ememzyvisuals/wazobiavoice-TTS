...
import numpy as np
import torch
from wazobiavoice_tts.mtl_tts import WazobiaVoiceMultilingualTTS, SUPPORTED_LANGUAGES
import gradio as gr
...
def get_or_load_model():
    """Loads the WazobiaVoiceMultilingualTTS model if it hasn't been loaded already,
    and ensures it's on the correct device."""
    global MODEL
    if MODEL is None:
        print("Model not loaded, initializing...")
        try:
            MODEL = WazobiaVoiceMultilingualTTS.from_pretrained(DEVICE, t3_model=T3_MODEL)
...
def generate_tts_audio(
    ...
    """
    Generate high-quality speech audio from text using WazobiaVoice Multilingual model with optional reference audio styling.
...
with gr.Blocks() as demo:
    gr.Markdown(
        """
        # WazobiaVoice Multilingual Demo
    ...
    with gr.Row():
        with gr.Column():
            ...
            language_id = gr.Dropdown(
                choices=list(WazobiaVoiceMultilingualTTS.get_supported_languages().keys()),
                value=initial_lang,
                label="Language",
